...
DB_PATH = ROOT / 'bilingual_calls.db'
...
def init_db():
    with database() as con:
        con.execute('''CREATE TABLE IF NOT EXISTS demo_calls (
            ...
            error TEXT, started REAL, sms_state TEXT, sms_sid TEXT, language TEXT DEFAULT 'en', menu_tries INTEGER DEFAULT 0)''')
...
def extract(speech, previous=None):
    ...
    model = config().get('GEMINI_MODEL') or 'gemini-3.5-flash-lite'
    ...
    instructions = (
        'Extract crop and weight from English or Telugu speech for a harvest demo. Return only the requested JSON. ' 'Map బియ్యం/biyyam to Rice, వరి/వడ్లు/ధాన్యం/vadlu to Paddy, ఉల్లిపాయలు to Onion, టమాటాలు to Tomato, మొక్కజొన్న to Maize, గోధుమలు to Wheat, పాలకూర to Spinach. Telugu number words must become numeric kg. '
...
def recommend(fields, language="en"):
    ...
    if language == 'te':
        crop_te = {'Rice':'బియ్యం','Paddy':'వడ్లు','Tomato':'టమాటాలు','Onion':'ఉల్లిపాయలు','Maize':'మొక్కజొన్న','Wheat':'గోధుమలు','Spinach':'పాలకూర'}[crop]
        buyer = 'నమూనా స్థానిక కొనుగోలుదారు' if best['transport'] == 0 else 'నమూనా టోకు కొనుగోలుదారు'
        return (f'మీ {kg:g} కిలోల {crop_te} కోసం, {buyer} సరిపోతారు. '
                f'ఈ డెమోలో ఊహించిన ధర కిలోకు {best["price"]} రూపాయలు. '
                f'రవాణా ఖర్చు {best["transport"]} రూపాయలు తీసివేస్తే, అంచనా మొత్తం {net:g} రూపాయలు. '
                'ఇవి నమూనా వివరాలు మాత్రమే. నిజమైన మార్కెట్ ధరలు లేదా బుకింగ్ కాదు.')
